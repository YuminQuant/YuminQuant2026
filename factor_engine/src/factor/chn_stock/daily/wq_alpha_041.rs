...
pub struct StockDailyWQAlpha041;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha041)
}

impl Factor for StockDailyWQAlpha041 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha041".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha041".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily", "deprecated"]
                ...
                .collect(),
            description: "WQAlpha041".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let geo = panel
            .column("high")?
            .zip_binary(&panel.column("low")?, |high, low| {
                mul(high, low).and_then(|value| (value >= 0.0).then_some(value.sqrt()))
            })?;
        let factor = geo.zip_binary(&vwap, sub)?;
