...
pub struct StockDailyWQAlpha083;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha083)
}

impl Factor for StockDailyWQAlpha083 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha083".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha083".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let close_mean5 = panel.column("close")?.ts(|values| ts_mean(values, 5, 5))?;
        let ratio = high_low.zip_binary(&close_mean5, div)?;
        let numerator = ratio
            .ts(|values| ts_delay(values, 2))?
        ...
        let denominator = ratio.zip_binary(&vwap.zip_binary(&panel.column("close")?, sub)?, div)?;
