...
pub struct StockDailyWQAlpha024;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha024)
}

impl Factor for StockDailyWQAlpha024 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha024".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha024".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha024".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let mean_100 = close.ts(|values| ts_mean(values, 100, 100))?;
        let mean_delta = mean_100.ts(|values| ts_diff(values, 100))?;
        let delay_close = close.ts(|values| ts_delay(values, 100))?;
        let ratio = mean_delta.zip_binary(&delay_close, div)?;
        let min_100 = close.ts(|values| ts_min(values, 100, 100))?;
        let close_minus_min = close.zip_binary(&min_100, sub)?;
        let delta_3 = close.ts(|values| ts_diff(values, 3))?;
        let factor = ratio.zip_ternary(
            &close_minus_min,
            &delta_3,
            |ratio, close_minus_min, delta_3| match clean(ratio) {
                Some(ratio) if ratio <= 0.05 => clean(close_minus_min).map(|value| -value),
                Some(_) => clean(delta_3).map(|value| -value),
                None => None,
            },
        )?;
