...
pub struct StockDailyWQAlpha026;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha026)
}

impl Factor for StockDailyWQAlpha026 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha026".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha026".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha026".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let volume_rank = panel.column("vol")?.ts(|values| ts_pctrank(values, 5, 5))?;
        let high_rank = panel
            .column("high")?
            .ts(|values| ts_pctrank(values, 5, 5))?;
        let corr = volume_rank.ts_binary(&high_rank, |volume, high| ts_corr(volume, high, 5, 5))?;
        let factor = corr
