# data_manager/downloaders/stock_daily_pv_downloader.py
...
class StockDailyPVDownloader(BaseDownloader):
    def __init__(self):
        config = ConfigManager().config
        
        # 严格使用新的 stock_daily 开头的配置名
        rate_limit = config['api']['rate_limits'].get('stock_daily_pv', 400)
        super().__init__(rate_limit=rate_limit)
        self.page_limit = config.get('api', {}).get('page_limits', {}).get('stock_daily_pv', 6000)
        
        # 路径初始化
        self.save_dir = self.get_full_path_and_ensure_dir('stock_daily_pv_dir')
        ...
        # 强制指定需要的字段，优化内存与网络传输
        self.fields = 'ts_code,trade_date,open,high,low,close,pre_close,change,pct_chg,vol,amount'
    ...
    def sync(self, start_date='20090101', target_end_date=None):
        ...
        self.logger.info(f"=== 开始同步 [股票日线量价(PV)数据] (区间: {start_date} -> {target_end_date}) ===")
        ...
        if not missing_dates:
            self.logger.info("本地日线PV数据已完全覆盖目标区间，无需更新。")
            return
            ...
            for date in tqdm(dates, desc=f"{year}年进度", mininterval=10.0, ascii=True):
                try:
                    ...
                    while True:
                        ...
                        self.logger.info(f"[{date}] 触发分页，获取第 {offset//self.page_limit + 1} 页...")
                        self.safe_sleep()
                        
                    self.safe_sleep()
                except Exception as e:
                    self.logger.error(f"拉取 {date} 日线PV数据失败: {e}")
            
            if yearly_new_data:
                self._save_yearly_data(year, yearly_new_data)
                
        self.logger.info("=== [股票日线量价(PV)数据] 同步结束 ===")
    ...
    def _save_yearly_data(self, year, new_data_list):
        ...
        # df_combined[['']] = df_combined['trade_date'].astype(np.int32)
