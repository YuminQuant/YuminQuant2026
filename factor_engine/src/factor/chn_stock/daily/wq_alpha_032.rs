...
pub struct StockDailyWQAlpha032;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha032)
}

impl Factor for StockDailyWQAlpha032 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha032".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha032".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily", "deprecated"]
                ...
                .collect(),
            description: "WQAlpha032".to_string(),
            dependencies: vec![DataRequest::new(
                DatasetId::StockDailyPv,
                &["close", "amount", "vol"],
            )],
            intraday_raw_dependencies: Vec::new(),
            lookback: Lookback { trading_days: 235 },
        }
    }
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let part1 = close
            .ts(|values| ts_mean(values, 7, 7))?
            .zip_binary(&close, sub)?
            .cs(cs_scale)?;
        let part2 = vwap
            .ts_binary(&close.ts(|values| ts_delay(values, 5))?, |vwap, close| {
                ts_corr(vwap, close, 230, 230)
            })?
            .cs(cs_scale)?
            .map_values(|value| clean(value).map(|value| value * 20.0));
        let factor = part1.zip_binary(&part2, add)?;
