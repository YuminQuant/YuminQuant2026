...
class StockDailyPVDownloader(BaseDownloader):
    ...
    def sync(self, start_date="20090101", target_end_date=None):
        ...
        self.logger.info(f"=== sync stock daily pv: {start_date} -> {target_end_date} ===")
        ...
        if not missing_dates:
            self.logger.info("stock daily pv is already complete for target range")
            return
        ...
        for year, dates in dates_by_year.items():
            daily_frames = []
            for date in tqdm(dates, desc=f"{year} stock_daily_pv", mininterval=10.0, ascii=True):
                try:
                    offset = 0
                    while True:
                    ...
                    self.logger.error(f"failed to fetch stock daily pv {date}: {exc}")
            if daily_frames:
                self._save_yearly_data(year, daily_frames)

        self.logger.info("=== stock daily pv sync complete ===")
