...
use crate::factor::chn_stock::daily::comprehensive_profitability::{
    ...
    ComprehensiveProfitabilityState, FCFFIC_ID, PROVIDER_KEY,
};
use crate::factor::{Factor, FactorUpdatePolicy};

pub struct StockDailyFcffic;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyFcffic)
}

impl Factor for StockDailyFcffic {
    fn spec(&self) -> FactorSpec {
        spec(ComprehensiveProfitabilityOutput::Fcffic)
    }
    ...
    fn compute(&self, context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        let requested = [FCFFIC_ID.to_string()];
        compute_requested(&requested, context, data)?
            .into_iter()
            .find(|series| series.spec.id == FCFFIC_ID)
            .ok_or_else(|| err("comprehensive profitability provider did not return fcffic"))
    }
