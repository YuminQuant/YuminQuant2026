...
use crate::factor::chn_stock::daily::comprehensive_profitability::{
    ...
    ComprehensiveProfitabilityState, PROVIDER_KEY, STABLE_ROIC_ID,
};
use crate::factor::{Factor, FactorUpdatePolicy};

pub struct StockDailyStableRoic;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyStableRoic)
}

impl Factor for StockDailyStableRoic {
    fn spec(&self) -> FactorSpec {
        spec(ComprehensiveProfitabilityOutput::StableRoic)
    }
    ...
    fn compute(&self, context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        let requested = [STABLE_ROIC_ID.to_string()];
        compute_requested(&requested, context, data)?
            .into_iter()
            .find(|series| series.spec.id == STABLE_ROIC_ID)
            .ok_or_else(|| err("comprehensive profitability provider did not return stable_roic"))
    }
