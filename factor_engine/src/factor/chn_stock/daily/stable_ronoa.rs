...
use crate::factor::chn_stock::daily::comprehensive_profitability::{
    ...
    ComprehensiveProfitabilityState, PROVIDER_KEY, STABLE_RONOA_ID,
};
use crate::factor::{Factor, FactorUpdatePolicy};

pub struct StockDailyStableRonoa;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyStableRonoa)
}

impl Factor for StockDailyStableRonoa {
    fn spec(&self) -> FactorSpec {
        spec(ComprehensiveProfitabilityOutput::StableRonoa)
    }
    ...
    fn compute(&self, context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        let requested = [STABLE_RONOA_ID.to_string()];
        compute_requested(&requested, context, data)?
            .into_iter()
            .find(|series| series.spec.id == STABLE_RONOA_ID)
            .ok_or_else(|| err("comprehensive profitability provider did not return stable_ronoa"))
    }
