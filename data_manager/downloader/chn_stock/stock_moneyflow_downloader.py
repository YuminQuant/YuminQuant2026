# data_manager/downloaders/stock_moneyflow_downloader.py
...
class StockMoneyflowDownloader(BaseDownloader):
    def __init__(self):
        ...
        self.page_limit = config.get('api', {}).get('page_limits', {}).get('stock_moneyflow', 6000)
    ...
    def sync(self, start_date='19901219', target_end_date=None):
        ...
        self.logger.info(f"=== 开始同步 [股票资金流向数据] (区间: {start_date} -> {target_end_date}) ===")
        ...
        if not missing_dates:
            self.logger.info("本地资金流向数据已完全覆盖目标区间，无需更新。")
            return
        ...
        for year, dates in dates_by_year.items():
            ...
            for date in tqdm(dates, desc=f"{year}年进度", mininterval=10.0, ascii=True):
                try:
                    ...
                    self.logger.error(f"拉取 {date} 资金流向数据失败: {e}")
            
            if yearly_new_data:
                self._save_yearly_data(year, yearly_new_data)
                
        self.logger.info("=== [股票资金流向数据] 同步结束 ===")
