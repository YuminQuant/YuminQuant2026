...
impl Label for StockDailyFutureOpenReturn5d {
    fn spec(&self) -> LabelSpec {
        LabelSpec {
            ...
            aliases: Vec::new(),
            name: "Stock future 5-day adjusted open-to-open".to_string(),
            ...
            .collect(),
            description: "Future adjusted open-to-open return from t+1 open to t+6 open."
                .to_string(),
            dependencies: vec![
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<LabelSeries> {
        ...
        let label = adjusted_open.ts(|values| future_return(values, 6))?;
