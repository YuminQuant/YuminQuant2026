...
pub struct StockDailyWQAlpha034;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha034)
}

impl Factor for StockDailyWQAlpha034 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha034".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha034".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha034".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let std_ratio = returns
            .ts(|values| ts_std_dev(values, 2, 2))?
            .zip_binary(&returns.ts(|values| ts_std_dev(values, 5, 5))?, div)?
            .cs(|values| cs_pctrank(values, true))?
            .map_values(|value| clean(value).map(|value| 1.0 - value));
        let delta_rank = close
            .ts(|values| ts_diff(values, 1))?
            .cs(|values| cs_pctrank(values, true))?
            .map_values(|value| clean(value).map(|value| 1.0 - value));
        let factor = std_ratio
            .zip_binary(&delta_rank, add)?
            .cs(|values| cs_pctrank(values, true))?;
