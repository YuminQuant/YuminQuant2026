...
impl StrategyEngine {
    ...
    pub fn run(&self, request: &StrategyRunRequest) -> Result<StrategyRunReport> {
        let mut run_config = StrategyRunConfig::load(
            &request.config_path,
            self.config.data_root.clone(),
            self.config.factor_root.clone(),
        )?;
        if let Some(detail) = request.detail {
            run_config.detail = detail;
        }
        let calendar =
        ...
        for trade_date in dates {
            ...
            let mut daily_snapshot = None::<HoldingSnapshot>;
            let mut daily_side_counts = SideCounts::default();
            for frame in frames {
                ...
                if !pending_orders.is_empty() {
                    ...
                    daily_side_counts.add_fills(&fills);
                ...
                let snapshot = holding_snapshot(&run_config, &account, &market, &bar_fills);
                if run_config.bar_frequency == BarFrequency::Minute && !run_config.detail {
                    daily_snapshot = Some(snapshot);
                } else {
                    holdings.push(snapshot);
                }
            }
            if run_config.bar_frequency == BarFrequency::Minute && !run_config.detail {
                if let Some(mut snapshot) = daily_snapshot {
                    daily_side_counts.apply_to_snapshot(&mut snapshot);
                    clear_trade_json(&mut snapshot);
                    holdings.push(snapshot);
                }
            }
...
fn load_frames(
    ...
    match (&config.asset_class, config.bar_frequency) {
        ...
        (StrategyAssetClass::Future, BarFrequency::Minute) => load_future_minute_frames(
            &config.data_root,
            &dates,
            future_meta,
...
fn holding_snapshot(
    ...
    let side_counts = SideCounts::from_fills(fills);
    ...
    HoldingSnapshot {
        ...
        account_pnl: round2(account.account_pnl()),
        realized_pnl_cum: round2(account.realized_pnl_cum()),
        net_realized_pnl_cum: round2(account.net_realized_pnl_cum()),
        unrealized_pnl: round2(account.total_unrealized_pnl()),
        gross_market_value: round2(account.gross_market_value()),
        net_market_value: round2(account.net_market_value()),
        margin_required: round2(account.margin_required()),
        available_margin: round2(account.available_margin()),
        position_count: symbols.len() as i64,
        buy_count: side_counts.buy,
        sell_count: side_counts.sell,
        short_count: side_counts.short,
        cover_count: side_counts.cover,
        ...
        market_values_json: json_f64_array_rounded(&market_values, 2),
        ...
        margin_values_json: json_f64_array_rounded(&margin_values, 2),
        ...
        trade_net_pnls_json: json_f64_array_rounded(
            &fills
                .iter()
...
impl SideCounts {
    fn from_fills(fills: &[FillEvent]) -> Self {
        let mut counts = Self::default();
        counts.add_fills(fills);
        counts
    }

    fn add_fills(&mut self, fills: &[FillEvent]) {
        for fill in fills {
            match fill.side {
                OrderSide::Buy => self.buy += 1,
                OrderSide::Sell => self.sell += 1,
                OrderSide::Short => self.short += 1,
                OrderSide::Cover => self.cover += 1,
            }
        }
    }

    fn apply_to_snapshot(self, snapshot: &mut HoldingSnapshot) {
        snapshot.buy_count = self.buy;
        snapshot.sell_count = self.sell;
        snapshot.short_count = self.short;
        snapshot.cover_count = self.cover;
    }
}

fn clear_trade_json(snapshot: &mut HoldingSnapshot) {
    snapshot.trade_symbols_json = "[]".to_string();
    snapshot.trade_sides_json = "[]".to_string();
    snapshot.trade_quantities_json = "[]".to_string();
    snapshot.trade_signed_quantities_json = "[]".to_string();
    snapshot.trade_prices_json = "[]".to_string();
    snapshot.trade_realized_pnls_json = "[]".to_string();
    snapshot.trade_net_pnls_json = "[]".to_string();
}
...
mod tests {
    ...
    fn holding_snapshot_uses_empty_json_arrays_without_positions_or_trades() {
        ...
        assert_eq!(snapshot.short_count, 0);
        assert_eq!(snapshot.cover_count, 0);
    ...
    fn holding_snapshot_records_positions_and_current_bar_trades_as_json() {
        ...
        assert_eq!(snapshot.market_values_json, "[1200.00]");
        ...
        assert_eq!(snapshot.margin_values_json, "[0.00]");
        ...
        assert_eq!(snapshot.trade_net_pnls_json, "[-1.00]");
    }

    #[test]
    fn clear_trade_json_keeps_side_counts_but_removes_details() {
        let mut account = AccountState::new(10_000.0);
        account.apply_buy("000001.SZ", 100.0, 10.0, 0.0);
        let fill = FillEvent {
            strategy_id: "s".to_string(),
            asset_class: "stock".to_string(),
            trade_date: 20200102,
        ...
        let mut snapshot = holding_snapshot(&config(), &account, &market(10.0), &[fill]);
        snapshot.buy_count = 7;
        super::clear_trade_json(&mut snapshot);

        assert_eq!(snapshot.buy_count, 7);
        assert_eq!(snapshot.trade_symbols_json, "[]");
        assert_eq!(snapshot.trade_sides_json, "[]");
        assert_eq!(snapshot.trade_net_pnls_json, "[]");
    }
}
