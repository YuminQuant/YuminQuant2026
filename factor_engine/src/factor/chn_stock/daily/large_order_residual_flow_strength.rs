...
pub struct StockDailyLargeOrderResidualFlowStrength;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyLargeOrderResidualFlowStrength)
}

impl Factor for StockDailyLargeOrderResidualFlowStrength {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "large_order_residual_flow_strength".to_string(),
            aliases: vec!["Large Order Residual Flow Strength".to_string()],
            name: "large_order_residual_flow_strength".to_string(),
            asset_class: AssetClass::Stock,
            frequency: Frequency::Daily,
            version: VERSION.to_string(),
            tags: tags(),
            description: "KYZQ large-order residual flow strength: rolling 20-day sum(net large buy-sell) over sum(abs(net)), residualized against adjusted Ret20 with intercept, then neutralized by Barra SIZE and SW sector.".to_string(),
            dependencies: vec![
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let buy = panel.column_from_table(moneyflow, "buy_lg_amount")?;
        let sell = panel.column_from_table(moneyflow, "sell_lg_amount")?;
...
mod tests {
    ...
    #[test]
    fn kyzq_large_flow_strength_uses_sum_net_over_sum_abs_net() {
    ...
    #[test]
    fn kyzq_large_flow_strength_rejects_zero_abs_flow() {
    ...
    #[test]
    fn kyzq_large_flow_strength_spec_has_kyzq_tag() {
        let spec = StockDailyLargeOrderResidualFlowStrength.spec();
        assert_eq!(spec.id, "large_order_residual_flow_strength");
