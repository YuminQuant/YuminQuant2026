...
class StockAdjFactorDownloader(BaseDownloader):
    def __init__(self):
        ...
        self.page_limit = config.get('api', {}).get('page_limits', {}).get('stock_adj_factor', 6000)
    ...
    def sync(self, start_date='20090101', target_end_date=None):
        ...
        self.logger.info(f"=== 开始同步 [股票复权因子] (区间: {start_date} -> {target_end_date}) ===")
        ...
        if not missing_dates:
            self.logger.info("本地复权因子已完全覆盖目标区间，无需更新。")
            return
        ...
        for year, dates in dates_by_year.items():
            self.logger.info(f"-> 处理 {year} 年复权因子...")
            yearly_new_data = []
            ...
            for date in tqdm(dates, desc=f"{year}年进度", mininterval=10.0, ascii=True):
                try:
                    ...
                    self.logger.error(f"拉取 {date} 复权因子失败: {e}")
            
            if yearly_new_data:
                self._save_yearly_data(year, yearly_new_data)
                
        self.logger.info("=== [股票复权因子] 同步结束 ===")
    ...
    def _save_yearly_data(self, year, new_data_list):
        ...
        self.logger.info(f"成功保存 {year}.parquet (已优化为 float32)，当前包含 {len(df_combined)} 条记录。")