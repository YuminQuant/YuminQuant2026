...
class StockDailyLimitDownloader(BaseDownloader):
    def __init__(self):
        ...
        self.fields = "trade_date,ts_code,up_limit,down_limit"
    ...
    def sync(self, start_date="19901219", target_end_date=None):
        ...
        self.logger.info(f"=== sync stock daily limit: {start_date} -> {target_end_date} ===")
        ...
        if not missing_dates:
            self.logger.info("stock daily limit is already complete for target range")
            return
        ...
        for year, dates in dates_by_year.items():
            daily_frames = []
            for date in tqdm(dates, desc=f"{year} stock_daily_limit", mininterval=10.0, ascii=True):
                try:
                    offset = 0
                    while True:
                    ...
                    self.logger.error(f"failed to fetch stock daily limit {date}: {exc}")
            if daily_frames:
                self._save_yearly_data(year, daily_frames)

        self.logger.info("=== stock daily limit sync complete ===")
    ...
    def _save_yearly_data(self, year, new_data_list):
        written = save_daily_frames(
            ...
            float32_cols=["up_limit", "down_limit"],
        )
