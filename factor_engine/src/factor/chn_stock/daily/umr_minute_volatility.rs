...
use crate::factor::common::{
    clean_intraday_value, intraday_time_in_range, stock_derived_bar_raw_spec, PanelColumn,
};
use crate::factor::common::{umr, DailyPanel};
use crate::factor::Factor;

const RAW_ID: &str = "daily_umr_minute_volatility";
const RAW_VERSION: &str = "0.2.0";
const VERSION: &str = "0.1.0";
const FIVE_MINUTE_BAR_SIZE: usize = 5;
const FIVE_MINUTE_BARS: usize = 48;

pub struct StockDailyUmrMinuteVolatility;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyUmrMinuteVolatility)
}
...
impl Factor for StockDailyUmrMinuteVolatility {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "umr_minute_volatility".to_string(),
            aliases: vec![
                "UMR_MINUTE_VOLATILITY".to_string(),
                "Minute Volatility-Adjusted UMR".to_string(),
            ],
            name: "Minute Volatility-Adjusted UMR".to_string(),
            asset_class: AssetClass::Stock,
            frequency: Frequency::Daily,
            version: VERSION.to_string(),
            tags: tags(),
            description: "Unified momentum and reversal factor adjusted by intraday 5-minute close-to-close return volatility, neutralized by Barra SIZE and SW sector.".to_string(),
            dependencies: vec![
        ...
        for trade_date in &context.target_dates {
            let daily_values = match data.derived_bar(FIVE_MINUTE_BAR_SIZE, *trade_date) {
                Some(table) => match raw_values_from_derived_bar(table) {
                    ...
                    Ok(None) | Err(_) => {
                        eprintln!(
                            "warning: umr_minute_volatility falling back to 1m minute data for {trade_date}; derived 5m bar is incomplete or incompatible"
                        );
                ...
                None => {
                    eprintln!(
                        "warning: umr_minute_volatility falling back to 1m minute data for {trade_date}; derived 5m bar is missing"
                    );
                    raw_values_from_minute(data, *trade_date)?
                }
            };
...
fn raw_values_from_minute(data: &DataPool, trade_date: i32) -> Result<Vec<(String, Option<f64>)>> {
    ...
    for (ts_code, mut indices) in grouped {
        ...
        output.push((ts_code, five_minute_return_std(&indices, &close)));
    }
    Ok(output)
}
    ...
    Ok(Some(
        grouped
            ...
            .map(|(ts_code, closes)| {
                ...
                let value = (returns.len() >= 2).then(|| std_dev(&returns));
                (ts_code, value)
            })
            .collect(),
    ))
...
fn five_minute_return_std(indices: &[usize], close: &[Option<f64>]) -> Option<f64> {
...
mod tests {
    ...
    #[test]
    fn five_minute_returns_use_close_to_close_after_first_five_minute_bar() {
        let indices = (0..10).collect::<Vec<_>>();
        let close = (1..=10).map(|value| Some(value as f64)).collect::<Vec<_>>();
        let returns = five_minute_close_returns(&indices, &close);
        assert_eq!(returns, vec![10.0 / 5.0 - 1.0]);
    }

    #[test]
    fn umr_minute_volatility_raw_spec_prefers_derived_bar_5m() {
        let spec = raw_spec();
        ...
        let factor = StockDailyUmrMinuteVolatility;
