...
class LightGBMOptunaAlphaModel(AlphaModel):
    ...
    def fit(self, train_data: pd.DataFrame, valid_data: pd.DataFrame, context: ModelContext) -> None:
        ...
        def objective(trial):
            params = {**base_params, **suggest_params(trial, search.get("space"), _default_lgbm_space())}
        ...
        self.model = lgb.LGBMRegressor(**self.best_params_)
...
def _default_lgbm_space() -> dict[str, Any]:
    return {
        ...
        "min_child_weight": {"type": "float", "low": 1e-4, "high": 100.0, "log": True},
...
def _lightgbm_module():
    try:
        ...
        raise ImportError("LightGBMOptunaAlphaModel requires installing lightgbm") from exc
    return lgb
...
def _optuna_module():
    try:
        ...
        raise ImportError("LightGBMOptunaAlphaModel requires installing optuna") from exc
    return optuna
