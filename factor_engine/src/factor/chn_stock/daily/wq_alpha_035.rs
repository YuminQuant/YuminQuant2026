...
pub struct StockDailyWQAlpha035;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha035)
}

impl Factor for StockDailyWQAlpha035 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha035".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha035".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha035".to_string(),
            dependencies: vec![DataRequest::new(
                DatasetId::StockDailyPv,
                &["close", "high", "low", "pre_close", "vol"],
            )],
            intraday_raw_dependencies: Vec::new(),
            lookback: Lookback { trading_days: 32 },
        }
    }

    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        let panel = data.daily_panel(DatasetId::StockDailyPv)?;
        let volume_rank = panel
            .column("vol")?
            .ts(|values| ts_pctrank(values, 32, 32))?;
        let price_range_rank = panel
            .column("close")?
            .zip_binary(&panel.column("high")?, add)?
            .zip_binary(&panel.column("low")?, sub)?
            .ts(|values| ts_pctrank(values, 16, 16))?
            .map_values(|value| clean(value).map(|value| 1.0 - value));
        let returns_rank = panel
            .column("close")?
            .zip_binary(&panel.column("pre_close")?, ret)?
            .ts(|values| ts_pctrank(values, 32, 32))?
            .map_values(|value| clean(value).map(|value| 1.0 - value));
        let factor = volume_rank
            .zip_binary(&price_range_rank, mul)?
            .zip_binary(&returns_rank, mul)?;
