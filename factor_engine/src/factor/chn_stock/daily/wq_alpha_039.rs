...
pub struct StockDailyWQAlpha039;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha039)
}

impl Factor for StockDailyWQAlpha039 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha039".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha039".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha039".to_string(),
            dependencies: vec![DataRequest::new(
                DatasetId::StockDailyPv,
                &["close", "pre_close", "vol"],
            )],
            intraday_raw_dependencies: Vec::new(),
            lookback: Lookback { trading_days: 250 },
        }
    }
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let vol_ratio_rank = volume
            .zip_binary(&adv20, div)?
            .ts(|values| ts_decay_linear(values, 9, 9))?
            .cs(|values| cs_pctrank(values, true))?
            .map_values(|value| clean(value).map(|value| 1.0 - value));
        let left = close
            .ts(|values| ts_diff(values, 7))?
            .zip_binary(&vol_ratio_rank, mul)?
            .cs(|values| cs_pctrank(values, true))?
        ...
        let right = returns
            ...
            .cs(|values| cs_pctrank(values, true))?
            .map_values(|value| clean(value).map(|value| 1.0 + value));
        let factor = left.zip_binary(&right, mul)?;
