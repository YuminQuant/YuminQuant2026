...
pub struct StockDailyWQAlpha060;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha060)
}

impl Factor for StockDailyWQAlpha060 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha060".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha060".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha060".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let left = close
            ...
            .cs(cs_scale)?
            .map_values(|value| clean(value).map(|value| 2.0 * value));
        let right = close
            .ts(|values| ts_argmax(values, 10, 10))?
            .cs(|values| cs_pctrank(values, true))?
            .cs(cs_scale)?;
        let factor = left
