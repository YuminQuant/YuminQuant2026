...
pub struct StockDailyWQAlpha027;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha027)
}

impl Factor for StockDailyWQAlpha027 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha027".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha027".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha027".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let rank_volume = volume.cs(|values| cs_pctrank(values, true))?;
        let rank_vwap = vwap.cs(|values| cs_pctrank(values, true))?;
        let corr = rank_volume.ts_binary(&rank_vwap, |volume, vwap| ts_corr(volume, vwap, 6, 6))?;
        let factor = corr
            .ts(|values| ts_mean(values, 2, 2))?
            .cs(|values| cs_pctrank(values, true))?
            .map_values(|value| match clean(value) {
