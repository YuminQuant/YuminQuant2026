...
class StockAdjFactorDownloader(BaseDownloader):
    def __init__(self):
        config = ConfigManager().config
        rate_limit = config["api"]["rate_limits"].get("stock_adj_factor", 400)
        super().__init__(rate_limit=rate_limit)
        self.page_limit = config.get("api", {}).get("page_limits", {}).get(
            "stock_adj_factor", 6000
        )
        self.save_dir = self.get_full_path_and_ensure_dir("stock_adj_factor_dir")
    ...
    def sync(self, start_date="20090101", target_end_date=None):
        ...
        self.logger.info(f"=== sync stock adj factor: {start_date} -> {target_end_date} ===")
        ...
        if not missing_dates:
            self.logger.info("stock adj factor is already complete for target range")
            return
        ...
        for year, dates in dates_by_year.items():
            daily_frames = []
            for date in tqdm(dates, desc=f"{year} stock_adj_factor", mininterval=10.0, ascii=True):
                try:
                    offset = 0
                    while True:
                        df_chunk = self.pro.adj_factor(
                            trade_date=date,
                    ...
                    self.logger.error(f"failed to fetch stock adj factor {date}: {exc}")
            if daily_frames:
                self._save_yearly_data(year, daily_frames)

        self.logger.info("=== stock adj factor sync complete ===")
