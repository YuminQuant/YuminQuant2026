...
pub struct StockDailyWQAlpha045;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha045)
}

impl Factor for StockDailyWQAlpha045 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha045".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha045".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha045".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let part1 = close
            .ts(|values| ts_delay(values, 5))?
            .ts(|values| ts_mean(values, 20, 20))?
            .cs(|values| cs_pctrank(values, true))?;
        let part2 = close.ts_binary(&volume, |close, volume| ts_corr(close, volume, 2, 2))?;
        let sum5 = close.ts(|values| ts_sum(values, 5, 5))?;
        let sum20 = close.ts(|values| ts_sum(values, 20, 20))?;
        let part3 = sum5
            .ts_binary(&sum20, |left, right| ts_corr(left, right, 2, 2))?
            .cs(|values| cs_pctrank(values, true))?;
        let factor = part1
            .zip_binary(&part2, mul)?
            .zip_binary(&part3, mul)?
