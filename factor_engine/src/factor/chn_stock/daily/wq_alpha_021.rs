...
pub struct StockDailyWQAlpha021;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha021)
}

impl Factor for StockDailyWQAlpha021 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha021".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha021".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha021".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let mean_8 = close.ts(|values| ts_mean(values, 8, 8))?;
        let mean_2 = close.ts(|values| ts_mean(values, 2, 2))?;
        let std_8 = close.ts(|values| ts_std_dev(values, 8, 8))?;
        let adv20 = volume.ts(|values| ts_mean(values, 20, 20))?;
        let vol_adv = volume.zip_binary(&adv20, div)?;
        let factor = mean_8.zip_quaternary(
            &std_8,
            &mean_2,
            &vol_adv,
            |mean_8, std_8, mean_2, vol_adv| match (
                clean(mean_8),
                clean(std_8),
                clean(mean_2),
                clean(vol_adv),
            ) {
                (Some(mean_8), Some(std_8), Some(mean_2), Some(vol_adv)) => {
                    if mean_8 + std_8 < mean_2 {
                        Some(-1.0)
                    } else if mean_2 < mean_8 - std_8 {
                        Some(1.0)
                    } else if vol_adv >= 1.0 {
                        Some(1.0)
                    } else {
