...
pub struct StockDailyWQAlpha037;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha037)
}

impl Factor for StockDailyWQAlpha037 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha037".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha037".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha037".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let open_close = panel
            .column("open")?
            .zip_binary(&panel.column("close")?, sub)?;
        let part1 = open_close
            .ts(|values| ts_delay(values, 1))?
            .ts_binary(&panel.column("close")?, |left, close| {
                ts_corr(left, close, 200, 200)
            })?
            .cs(|values| cs_pctrank(values, true))?;
        let part2 = open_close.cs(|values| cs_pctrank(values, true))?;
