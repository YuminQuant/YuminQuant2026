...
pub struct StockDailyWQAlpha022;

pub fn create() -> Box<dyn Factor> {
    Box::new(StockDailyWQAlpha022)
}

impl Factor for StockDailyWQAlpha022 {
    fn spec(&self) -> FactorSpec {
        FactorSpec {
            id: "WQAlpha022".to_string(),
            aliases: Vec::new(),
            name: "WQAlpha022".to_string(),
            ...
            tags: ["worldquant101alpha", "price_volume", "daily"]
                ...
                .collect(),
            description: "WQAlpha022".to_string(),
    ...
    fn compute(&self, _context: &FactorContext, data: &DataPool) -> Result<FactorSeries> {
        ...
        let corr = high.ts_binary(&volume, |high, volume| ts_corr(high, volume, 5, 5))?;
        let delta_corr = corr.ts(|values| ts_diff(values, 5))?;
        let ranked_std = close
            .ts(|values| ts_std_dev(values, 20, 20))?
            .cs(|values| cs_pctrank(values, true))?;
        let factor = delta_corr.zip_binary(&ranked_std, |delta_corr, ranked_std| {
            mul(delta_corr, ranked_std).map(|value| -value)
        })?;
