...
pub struct StockDailyFutureOpen10mVwapReturn20d;

pub fn create() -> Box<dyn Label> {
    Box::new(StockDailyFutureOpen10mVwapReturn20d)
}

impl Label for StockDailyFutureOpen10mVwapReturn20d {
    fn spec(&self) -> LabelSpec {
        LabelSpec {
            id: "future_open_10m_vwap_return_20d".to_string(),
            aliases: Vec::new(),
            name: "Stock future 20-day opening 10-minute adjusted VWAP".to_string(),
            ...
            .collect(),
            description:
                "Future adjusted return from t+1 09:31-09:40 VWAP to t+21 09:31-09:40 VWAP."
                    .to_string(),
            dependencies: vec![
    ...
    fn compute(&self, context: &FactorContext, data: &DataPool) -> Result<LabelSeries> {
        let adjusted_vwap = adjusted_opening_vwap(data, context, "09:31:00", "09:40:00")?;
