...
class HKMinuteDownloader(BaseDownloader):
    """港股分钟线下载器 (多线程并发 Batch 极速版)"""
    def __init__(self):
        config = ConfigManager().config
        super().__init__(rate_limit=config['api']['rate_limits'].get('hk_minute', 400))
        self.page_limit = config['api']['page_limits'].get('hk_minute', 8000)
        self.save_dir = self.get_full_path_and_ensure_dir('hk_minute_dir')
        self.daily_pv_dir = self.get_full_path_and_ensure_dir('hk_daily_dir')
    ...
    def _get_valid_stocks_for_day(self, date_str):
        ...
        if not os.path.exists(pv_file): return []
        try:
    ...
    def sync(self, start_date='20100101', target_end_date=None):
        ...
        self.logger.info(f"=== 开始同步 [港股分钟线] (多线程并发 Batch) ({start_date} -> {target_end_date}) ===")
    ...
    def _fetch_and_save_single_day(self, date, current_idx, total_days):
        ...
        if not valid_codes:
            self.logger.warning(f"[{date}] 未找到交易港股，跳过。")
            return
            
        # =========================================================
        # 港股核心：每天交易 330 分钟，Batch 最大容量必须降到 20 只！
        # 20 * 330 = 6600 < 8000
        # =========================================================
        batch_size = 20 
        ...
        start_time = f"{fmt_date} 09:00:00"
        end_time = f"{fmt_date} 16:30:00" # 放宽到 16:30 包含盘后竞价
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_batch):
                try:
                    ...
                    if isinstance(result, Exception): pass # 日志略去，同 A 股
        ...
        if day_chunks:
            ...
            for col in ['open', 'high', 'low', 'close', 'vol', 'amount']:
                if col in df_day.columns: df_day[col] = df_day[col].astype(np.float32)
            ...
            self.logger.info(f"✅ [{date}] 落盘 (含 {len(df_day)} 行) [大盘进度: {current_idx}/{total_days} | 剩余: {remaining} 天]")