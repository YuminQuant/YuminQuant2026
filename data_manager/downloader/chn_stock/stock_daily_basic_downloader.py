...
class StockDailyBasicDownloader(BaseDownloader):
    ...
    def sync(self, start_date="19901219", target_end_date=None):
        ...
        self.logger.info(f"=== sync stock daily basic: {start_date} -> {target_end_date} ===")
        ...
        if not missing_dates:
            self.logger.info("stock daily basic is already complete for target range")
            return
        ...
        for year, dates in dates_by_year.items():
            daily_frames = []
            for date in tqdm(dates, desc=f"{year} stock_daily_basic", mininterval=10.0, ascii=True):
                try:
                    offset = 0
                    while True:
                    ...
                    self.logger.error(f"failed to fetch stock daily basic {date}: {exc}")
            if daily_frames:
                self._save_yearly_data(year, daily_frames)

        self.logger.info("=== stock daily basic sync complete ===")
